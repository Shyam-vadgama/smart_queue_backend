...
@app.get("/customer/my_queue_status")
async def get_my_queue_status(db: Session = Depends(get_db), current_user: User = Depends(get_current_active_user)):
    """
    Gets the current active queue status for the logged-in customer.
    """
    active_queue = db.query(Queue).filter(
        Queue.CustomerID == current_user.UserID,
        Queue.Status.in_([Status.waiting, Status.in_service])
    ).first()

    if not active_queue:
        return {"in_queue": False}

    service = db.query(Service).filter(Service.ServiceID == active_queue.ServiceID).first()

    # Calculate position in queue
    position = 0
    if active_queue.Status == Status.waiting:
        position = db.query(Queue).filter(
            Queue.ServiceID == active_queue.ServiceID,
            Queue.Status == Status.waiting,
            Queue.CreatedAt < active_queue.CreatedAt
        ).count() + 1

    return {
        "in_queue": True,
        "queue_id": active_queue.TokenID,
        "service_id": active_queue.ServiceID,
        "service_name": service.ServiceName if service else "Unknown Service",
        "token_number": active_queue.TokenNumber,
